...
def main():
    ...
    st.markdown("Upload your API key and wallet address to fetch and filter NFTs by year (1990-2025) from your Solana wallet, view results directly, and export to CSV.")
    api_key = st.text_input("Enter your Helius API Key:", type="password")
    wallet_address = st.text_input("Enter your Solana Wallet Address:")
    if st.button("Fetch & Export NFTs"):
        if not api_key or not wallet_address:
            ...
            st.info("Fetching NFTs from wallet...")
            all_nfts = extractor.get_nft_assets()
            if not all_nfts:
                st.warning("No NFTs found or error occurred.")
            else:
                st.success(f"Found {len(all_nfts)} NFTs. Filtering by year 1990-2025...")
                ...
                if not filtered_nfts:
                    ...
                    st.success(f"Found {len(filtered_nfts)} NFTs matching criteria.")
